	...
	async def execute(self, context: ActionContext) -> ActionResult:
		...
		if change_detected:
			return ActionResult(
				...
				message=f'Clicked on coordinates ({self.x}, {self.y}). Change detected: {change_type}. The mouse pointer is visible in the screenshot.',
			...
			return ActionResult(
				...
				message=f'Clicked on coordinates ({self.x}, {self.y}). No obvious changes detected. Adjust the coordinates, and try again. Do not try again with the same coordinates. Try adding/removing a few pixels up or down, left or right. Analyze the position of the cursor in the screenshot, that is the position the click was made.',
				context_change=ContextChange(action=ContextChangeTypes.SCREENSHOT, data={'screenshot': screenshot}),
			)
